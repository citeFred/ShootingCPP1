...
APlayerPawn::APlayerPawn()
{
	...
	// 충돌 채널 설정을 C++에서 직접 하는경우
	// 충돌 설정은 채널/응답 조합이 복잡해질 가능성이 높기때문에, 언리얼에디터의 프리셋으로 관리하는것이 유리함(관례적)
	// boxComp->SetCollisionObjectType(ECollisionChannel::ECC_GameTraceChannel1); // Player 채널
...
void APlayerPawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	{
		// BindAction() 함수 구성
		// BindAction(IA이름, 입력이벤트타입, 연결할함수가있는클래스, 연결할함수의 주소값)
